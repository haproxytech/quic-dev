...
static int qc_prep_hdshk_pkts(struct qring *qr, struct ssl_sock_ctx *ctx)
{
 ...
 start:
	...
	while (end_buf - pos >= (int)qc->path->mtu + dg_headlen || prv_pkt) {
		 ...
		 */
		do {
			nb_pto_dgrams = HA_ATOMIC_LOAD(&qc->tx.nb_pto_dgrams);
		} while (nb_pto_dgrams &&
		         HA_ATOMIC_CAS(&qc->tx.nb_pto_dgrams, &nb_pto_dgrams,
		                       nb_pto_dgrams - 1));
		if (!(qel->tls_ctx.tx.flags & QUIC_FL_TLS_SECRETS_SET) ||
		    (!(qel->pktns->flags & QUIC_FL_PKTNS_ACK_REQUIRED) && !nb_pto_dgrams &&
		...
		cur_pkt = qc_build_pkt(&pos, end, qel, qc, pkt_type, &nb_pto_dgrams, &err);
		switch (err) {
		case -2:
			goto err;
		case -1:
