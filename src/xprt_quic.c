...
struct quic_version quic_versions[] = {
	{
 ...
 */
static struct quic_conn *qc_new_conn(struct quic_version *qv, int ipv4,
 ...
 */
static inline struct quic_version *qc_pkt_is_supported_version(uint32_t version)
{
	int i;
...
static int send_retry(int fd, struct sockaddr_storage *addr,
                      struct quic_rx_packet *pkt, struct quic_version *qv)
{
	unsigned char buf[128];
	int i = 0, token_len;
	const socklen_t addrlen = get_addr_len(addr);
                             ...
                             struct quic_dgram *dgram)
{
	...
	if (long_header) {
		...
		qv = qc_pkt_is_supported_version(version);
		if (!qv) {
