	...
	if (qc_local_stream_id(qcc, id)) {
		...
		if (sub_id > (int64_t)largest_id) {
			...
			for (i = largest_id + 1; i <= sub_id; i++) {
				stream = qcs_new(qcc, (i << QCS_ID_TYPE_SHIFT) | strm_type);
				if (!stream) {
				...
				qcc->strms[qcs_id_type(id)].largest_id = i;
			}

			if (id & QCS_ID_DIR_BIT) {
				struct iuqcs *iuqcs = stream;
				strm_node = &iuqcs->by_id;
			}
			else {
				struct qcs *qcs = stream;
				strm_node = &qcs->by_id;
			}
		}
		else {
 ...
 * Returns the number of bytes copied or -1 if
 */
 ...
 * Return 1 if succeeded, 0 if not.
 */
static int qc_handle_bidi_stream_frm(struct quic_rx_packet *pkt,
                                     ...
                                     struct quic_conn *qc)
{
	...
	if (strm_frm->offset == strm->rx.offset) {
		if (!qc_get_buf(qc->qcc, &strm->rxbuf))
		    goto store_frm;

		qc_strm_cpy(&strm->rxbuf, strm_frm);
		qc->qcc->app_ops->decode_qcs(strm, qc->qcc->ctx);
	}
 ...
 * Return 1 if succeeded, 0 if not.
 */
static int qc_handle_uni_stream_frm(struct quic_rx_packet *pkt,
                                    struct quic_stream *strm_frm,
                                    struct quic_conn *qc)
{
	struct iuqcs *strm;
	...
	strm = eb64_entry(&strm_node->node, struct iuqcs, by_id);
	frm_node = eb64_lookup(&strm->frms, strm_frm->offset);
	/* FIXME: handle the case where this frame overlap others */
	if (frm_node) {
		TRACE_PROTO("Already existing stream data",
	...
	if (strm_frm->offset == strm->rx.offset) {
		...
		qc_strm_cpy(&strm->rxbuf, strm_frm);
		qc->qcc->app_ops->decode_iuqcs(strm, qc->qcc->ctx);
	}
...
static inline int qc_handle_stream_frm(struct quic_rx_packet *pkt,
                                       struct quic_stream *strm_frm,
                                       struct quic_conn *qc)
{
	if (strm_frm->id & QCS_ID_DIR_BIT)
		return qc_handle_uni_stream_frm(pkt, strm_frm, qc);
	else
		return qc_handle_bidi_stream_frm(pkt, strm_frm, qc);
}
                             ...
                             struct quic_enc_level *qel)
{
	...
	while (pos < end) {
		...
		case QUIC_FT_STREAM_F:
		{
			...
			if (!qc_handle_stream_frm(pkt, stream, ctx->conn->qc))
				goto err;
