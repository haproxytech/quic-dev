	...
	if (conn) {
		...
		if (mask & QUIC_EV_CONN_HPKT) {
			...
			if (qel) {
				...
				pktns = qc->pktns;
				chunk_appendf(&trace_buf, " qel=%c cwnd=%llu ppif=%lld pif=%lu "
			...
			if (pkt) {
				const struct quic_tx_frm *frm;
				chunk_appendf(&trace_buf, " pn=%lu cdlen=%lu",
		...
		if (mask & (QUIC_EV_CONN_ELRXPKTS|QUIC_EV_CONN_PRSHPKT|QUIC_EV_CONN_SSLDATA)) {
			...
			if (pkt)
				chunk_appendf(&trace_buf, " pkt@%p el=%c pn=%lu", pkt,
		...
		if (mask & QUIC_EV_CONN_PHPKTS) {
			...
			if (qel) {
				...
				pktns = qc->pktns;
				chunk_appendf(&trace_buf,
				              " qel=%c ack?%d cwnd=%llu ppif=%lld pif=%llu if=%lu pp=%u pdg=%llu",
				              quic_enc_level_char_from_qel(qel, qc),
