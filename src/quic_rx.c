                             ...
                             struct quic_dgram *dgram, enum obj_type *o)
{
	...
	struct quic_counters *prx_counters;
	struct listener *l = *o == OBJ_TYPE_LISTENER ? __objt_listener(o) : NULL;
	...
	if (pkt->type != QUIC_PACKET_TYPE_SHORT) {
		...
		if (pkt->type == QUIC_PACKET_TYPE_INITIAL) {
			 ...
			 * suspect network usage is detected. Retry packet are sent only
			 * by servers.
			 */
			if (!token_len) {
				if (l && l->bind_conf->options & BC_O_QUIC_FORCE_RETRY) {
					TRACE_PROTO("Initial without token, sending retry",
					            QUIC_EV_CONN_LPKT, NULL, NULL, NULL, pkt->version);
					if (send_retry(l->rx.fd, &dgram->saddr, pkt, pkt->version)) {
                     ...
                     enum obj_type *o)
{
	...
	do {
		...
		if (!qc) {
			int new_tid = -1;
			struct listener *li = *o == OBJ_TYPE_LISTENER ? __objt_listener(o) : NULL;
