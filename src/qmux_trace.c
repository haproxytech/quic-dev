...
{
	chunk_appendf(msg, " qcs=%p .id=%llu .st=%s .fl=%#08x", qcs, (ullong)qcs->id,
	...
	chunk_appendf(msg, " .ti=%ums/%us/%ums",
	              qmux_ti_read(&qcs->timeinfo.base),
	              qmux_ti_read(&qcs->timeinfo.buf),
	              qmux_ti_read(&qcs->timeinfo.fctl));
}
