...
int qpack_decode_enc(struct iuqcs *qcs, void *ctx)
{
	size_t len;
	struct buffer *rxbuf;
	unsigned char inst;
...
int qpack_decode_dec(struct iuqcs *qcs, void *ctx)
{
	size_t len;
	struct buffer *rxbuf;
	unsigned char inst;
...
int qpack_decode_fs(const unsigned char *raw, size_t len, struct buffer *tmp)
{
	...
	qpack_debug_hexdump(stderr, "[QPACK-DEC-FS] ", raw, 0, len);
	...
	/* Decode field lines */
	while (len) {
		qpack_debug_hexdump(stderr, "raw ", raw, 0, len);
		...
		if (efl_type == QPACK_LFL_WPBNM) {
			...
			qpack_debug_printf(stderr, " n=%d index=%llu", !!n, index);
			h = *raw & 0x80;
			...
			qpack_debug_printf(stderr, " h=%d length=%llu", !!h, length);
			...
			qpack_debug_printf(stderr, " index=%llu", index);
			...
			qpack_debug_printf(stderr,  " t=%d index=%llu", !!t, index);
			...
			qpack_debug_printf(stderr, " n=%d t=%d index=%llu", !!n, !!t, index);
			h = *raw & 0x80;
			length = qpack_get_varint(&raw, &len, 7);
			if (len == (uint64_t)-1) {
				qpack_debug_printf(stderr, "##ERR@%d\n", __LINE__);
			...
			qpack_debug_printf(stderr, " n=%d hanme=%d name_len=%llu", !!n, !!hname, name_len);
			/* Name string */
			raw += name_len;
			len -= name_len;
			hvalue = *raw & 0x80;
			...
			qpack_debug_printf(stderr, " hvalue=%d value_len=%llu", !!hvalue, value_len);
