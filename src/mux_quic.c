...
/* the qcs bidirection stream pool */
DECLARE_POOL(pool_head_qcs, "qcs", sizeof(struct qcs));
/* the unidirectional stream pool for input data */
DECLARE_POOL(pool_head_iuqcs, "iuqcs", sizeof(struct iuqcs));
/* the unidirectional stream pool for output data*/
DECLARE_POOL(pool_head_ouqcs, "ouqcs", sizeof(struct ouqcs));
...
static void qc_release(struct qcc *qcc)
{
	...
	if (qcc) {
		...
		if (LIST_ADDED(&qcc->buf_wait.list))
			LIST_DEL(&qcc->buf_wait.list);
...
void ouqcs_release(struct ouqcs *qcs)
{
	eb64_delete(&qcs->by_id);
	pool_free(pool_head_ouqcs, qcs);
}

/* Allocates a locally initiated unidirectional stream. */
struct ouqcs *ouqcs_new(struct qcc *qcc)
{
	struct ouqcs *qcs;
	enum qcs_type qcs_type;
	uint64_t next_id;
	...
	qcs = pool_alloc(pool_head_ouqcs);
	if (!qcs)
		goto out;

	qcs->qcc = qcc;
	qcs->by_id.key = next_id;
	qcs->tx.st = QC_TX_SS_IDLE;
	qcs->tx.max_data = qcc->strms[qcs_type].tx.max_data;
...
struct iuqcs *iuqcs_new(struct qcc *qcc, uint64_t id)
{
	struct iuqcs *qcs;
	enum qcs_type qcs_type;

	TRACE_ENTER(QC_EV_QCS_NEW, qcc->conn);
	qcs = pool_alloc(pool_head_iuqcs);
	if (!qcs)
		goto out;
...
/* attempt to notify the data layer of recv availability */
void iuqcs_notify_recv(struct iuqcs *qcs)
{
	if (qcs->subs && qcs->subs->events & SUB_RETRY_RECV) {
 ...
 */
static int iuqcs_subscribe(struct iuqcs *qcs, int event_type, struct wait_event *es)
{
	struct qcc *qcc = qcs->qcc;
 ...
 * subscribe() call. It always returns zero.
 */
static int iuqcs_unsubscribe(struct iuqcs *qcs, int event_type, struct wait_event *es)
{
	struct qcc *qcc = qcs->qcc;
 ...
 */
static int ouqcs_subscribe(struct ouqcs *qcs, int event_type, struct wait_event *es)
{
	struct qcc *qcc = qcs->qcc;
 ...
 * subscribe() call. It always returns zero.
 */
static int ouqcs_unsubscribe(struct ouqcs *qcs, int event_type, struct wait_event *es)
{
	struct qcc *qcc = qcs->qcc;
 ...
 */
static size_t _ouqcs_snd_buf(struct ouqcs *qcs, struct buffer *buf, size_t count, int flags)
{
	size_t total = 0;
	struct qcc *qcc = qcs->qcc;
	struct buffer *res;
 ...
 */
size_t ouqcs_snd_buf(struct ouqcs *qcs, struct buffer *buf, size_t count, int flags)
{
	size_t room, total = 0;
	struct qcc *qcc = qcs->qcc;
	struct buffer *res;
...
static const struct mux_ops qc_ops = {
	...
	.iuqcs_subscribe = iuqcs_subscribe,
	.iuqcs_unsubscribe = iuqcs_unsubscribe,
	.ouqcs_subscribe = ouqcs_subscribe,
	.ouqcs_unsubscribe = ouqcs_unsubscribe,
	.attach = qc_attach,
