...
static const struct qcs *qc_error_stream;
static const struct qcs *qc_refused_stream;
static const struct qcs *qc_idle_stream;
 ...
 * received data.
 */
#define QC_CF_DEM_DALLOC        0x00000004  // demux blocked on lack of connection's demux buffer
#define QC_CF_DEM_DFULL         0x00000008  // demux blocked on connection's demux buffer full

#define QC_CF_DEM_MBUSY         0x00000010  // demux blocked on connection's mux side busy
#define QC_CF_DEM_MROOM         0x00000020  // demux blocked on lack of room in mux buffer
#define QC_CF_DEM_SALLOC        0x00000040  // demux blocked on lack of stream's request buffer
#define QC_CF_DEM_SFULL         0x00000080  // demux blocked on stream request buffer full
#define QC_CF_DEM_TOOMANY       0x00000100  // demux blocked waiting for some conn_streams to leave
#define QC_CF_DEM_BLOCK_ANY     0x000001F0  // aggregate of the demux flags above except DALLOC/DFULL
...
static struct task *qc_timeout_task(struct task *t, void *context, unsigned short state);
static int qc_send(struct qcc *qcc);
static int qc_recv(struct qcc *qcc);
static int qc_process(struct qcc *qcc);
static struct task *qc_io_cb(struct task *t, void *ctx, unsigned short state);
static inline struct qcs *qcc_st_by_id(struct qcc *qcc, int id);
static int qc_frt_transfer_data(struct qcs *qcs);
static struct task *qc_deferred_shut(struct task *t, void *ctx, unsigned short state);
...
{
	if (conn_xprt_read0_pending(qcc->conn) && !b_data(&qcc->dbuf))
		return 1;
	return 0;
}
...
{
	if (b_data(&qcc->dbuf) == 0 &&
	    (qcc->st0 >= QC_CS_ERROR ||
	     qcc->conn->flags & CO_FL_ERROR ||
	     conn_xprt_read0_pending(qcc->conn)))
		return 0;

	if (!(qcc->flags & QC_CF_DEM_DALLOC) &&
	    !(qcc->flags & QC_CF_DEM_BLOCK_ANY))
		return 1;
...
static inline void qcc_restart_reading(const struct qcc *qcc, int consider_buffer)
{
	...
	if ((!consider_buffer || !b_data(&qcc->dbuf))
...
static int qc_buf_available(void *target)
{
	...
	if ((qcc->flags & QC_CF_DEM_DALLOC) && b_alloc_margin(&qcc->dbuf, 0)) {
		qcc->flags &= ~QC_CF_DEM_DALLOC;
...
static inline struct buffer *qc_get_buf(struct qcc *qcc, struct buffer *bptr)
{
	struct buffer *buf = NULL;

	if (likely(!MT_LIST_ADDED(&qcc->buf_wait.list)) &&
	    unlikely((buf = b_alloc_margin(bptr, 0)) == NULL)) {
		qcc->buf_wait.target = qcc;
		qcc->buf_wait.wakeup_cb = qc_buf_available;
		MT_LIST_ADDQ(&buffer_wq, &qcc->buf_wait.list);
	}
	return buf;
}
                   ...
                   struct session *sess, struct buffer *input)
{
	...
	struct quic_transport_params *clt_params, *srv_params;
	...
	MT_LIST_INIT(&qcc->buf_wait.list);
...
static void qc_release(struct qcc *qcc)
{
	...
	if (qcc) {
		...
		if (MT_LIST_ADDED(&qcc->buf_wait.list))
			MT_LIST_DEL(&qcc->buf_wait.list);

		qc_release_buf(qcc, &qcc->dbuf);
		qc_release_mbuf(qcc);
...
static void __maybe_unused qcs_notify_recv(struct qcs *qcs)
{
...
static void __maybe_unused qcs_notify_send(struct qcs *qcs)
{
 ...
 * conn_stream anymore.
 */
static void __maybe_unused qcs_alert(struct qcs *qcs)
{
...
/* allocates a new stream <id> for connection <qcc> and adds it into qcc's
                                      ...
                                      struct conn_stream *cs, struct session *sess)
{
	...
	qcs = qcs_new(qcc, qcc->strms[qcs_type].largest_id + 1);
	if (!qcs)
		goto out;
...
static struct task *qc_io_cb(struct task *t, void *ctx, unsigned short status)
{
	...
	if (event_type & SUB_RETRY_SEND) {
		...
		if (!(qcs->flags & QC_SF_BLK_SFCTL) &&
			...
			if (qcs->flags & QC_SF_BLK_MFCTL)
				LIST_ADDQ(&qcc->fctl_list, &qcs->list);
			else
				LIST_ADDQ(&qcc->send_list, &qcs->list);
		}
	}
...
static void qc_show_fd(struct buffer *msg, struct connection *conn)
{
	struct qcc *qcc = conn->ctx;
	struct qcs *qcs = NULL;
	struct eb64_node *node;
	...
	chunk_appendf(msg, " qcc.st0=%s .flg=0x%04x"
		      ...
		      (unsigned int)b_data(&qcc->dbuf), b_orig(&qcc->dbuf),
		      (unsigned int)b_head_ofs(&qcc->dbuf), (unsigned int)b_size(&qcc->dbuf),
	...
	if (qcs) {
		chunk_appendf(msg, " last_qcs=%p .id=%d rx.st=%s tx.st=%s .flg=0x%04x .rxbuf=%u@%p+%u/%u .cs=%p",
			      qcs, qcs->id, qcs_rx_st_to_str(qcs->rx.st), qcs_tx_st_to_str(qcs->tx.st), qcs->flags,
			      (unsigned int)b_data(&qcs->rxbuf), b_orig(&qcs->rxbuf),
			      (unsigned int)b_head_ofs(&qcs->rxbuf), (unsigned int)b_size(&qcs->rxbuf),
			      qcs->cs);
		if (qcs->cs)
