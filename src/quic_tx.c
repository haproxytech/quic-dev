                                           ...
                                           struct list *frms)
{
	...
	qc->ael->tmp_send_frms = frms;
	ret = qc_send_pkts(qc, 1, qc->ael, NULL);
...
int qc_send_mux(struct quic_conn *qc, struct list *frms)
{
	...
	if ((qc->flags & QUIC_FL_CONN_NEED_POST_HANDSHAKE_FRMS) &&
		...
		qc->ael->tmp_send_frms = &qc->ael->pktns->tx.frms;
		qc_send_pkts(qc, 0, qc->ael, NULL);
	}

	TRACE_STATE("preparing data (from MUX)", QUIC_EV_CONN_TXPKT, qc);
	qc->flags |= QUIC_FL_CONN_TX_MUX_CONTEXT;
	qc->ael->tmp_send_frms = frms;
	ret = qc_send_pkts(qc, 0, qc->ael, NULL);
	qc->flags &= ~QUIC_FL_CONN_TX_MUX_CONTEXT;
 ...
 */
int qc_prep_hpkts(struct quic_conn *qc, struct buffer *buf,
                  struct qel_iter *qels, int old_data)
{
	int ret, cc, padding;
	...
	while ((qel = qel_iter_next(qels))) {
		struct quic_tls_ctx *tls_ctx;
		const struct quic_version *ver;
		struct list *frms, *next_frms = NULL;
		...
		frms = qel_iter_frms(qels, qel);

		next_qel = qel_iter_next_peek(qels);
		next_frms = next_qel ? qel_iter_frms(qels, next_qel) : NULL;
...
/* Emit packets for <qc> QUIC connection. If either <qel1> and/or <qel2> is set,
 * sending will loop over them using their external frame attach point. If neither of
 * them is set, sending will be performed over all <qc> registered encryption
 * levels using frames stored in their respective pktns. <old_data> must be set
 * if data are reemitted for data not acknowledged.
 *
 * Returns 1 on success else 0.
 */
int qc_send_pkts(struct quic_conn *qc, int old_data,
                 struct quic_enc_level *qel1, struct quic_enc_level *qel2)
{
	...
	if (qel1) {
		BUG_ON(LIST_INLIST(&qel1->tmp_send));
		LIST_APPEND(&qels, &qel1->tmp_send);
	}

	if (qel2) {
		BUG_ON(LIST_INLIST(&qel2->tmp_send));
		LIST_APPEND(&qels, &qel2->tmp_send);
	}

	qels_iter = LIST_ISEMPTY(&qels) ?
	  qel_iter_new(&qc->qel_list, 0) : qel_iter_new(&qels, 1);

	ret = qc_prep_hpkts(qc, buf, &qels_iter, old_data);
	if (ret == -1) {
		qc_txb_release(qc);
 ...
 out:
	if (qel1) {
		LIST_DEL_INIT(&qel1->tmp_send);
		qel1->tmp_send_frms = NULL;
	}

	if (qel2) {
		LIST_DEL_INIT(&qel2->tmp_send);
		qel2->tmp_send_frms = NULL;
	}
	...
	if (ipktns && (ipktns->flags & QUIC_FL_PKTNS_PROBE_NEEDED)) {
		...
		for (i = 0; i < QUIC_MAX_NB_PTO_DGRAMS; i++) {
			...
			if (!LIST_ISEMPTY(&ifrms)) {
				...
				qc->iel->tmp_send_frms = &ifrms;
				if (qc->hel)
					qc->hel->tmp_send_frms = &hfrms;
				sret = qc_send_pkts(qc, 1, qc->iel, qc->hel);
				...
				ipktns->tx.pto_probe = 1;
				qc->iel->tmp_send_frms = &ifrms;
				sret = qc_send_pkts(qc, 0, qc->iel, NULL);
				qc_free_frm_list(qc, &ifrms);
				qc_free_frm_list(qc, &hfrms);
				if (!sret)
					goto leave;
		...
		if (hpktns && (hpktns->flags & QUIC_FL_PKTNS_PROBE_NEEDED)) {
			...
			for (i = 0; i < QUIC_MAX_NB_PTO_DGRAMS; i++) {
				...
				if (!LIST_ISEMPTY(&frms1)) {
					hpktns->tx.pto_probe = 1;
					qc->hel->tmp_send_frms = &frms1;
					sret = qc_send_pkts(qc, 1, qc->hel, NULL);
