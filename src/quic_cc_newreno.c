...
static void quic_cc_nr_ca_cb(struct quic_cc *cc, struct quic_cc_event *ev)
{
	...
	case QUIC_CC_EVT_ACK:
		 ...
		 * congestion window.
		 */
		cc->algo_state.nr.cwnd += path->mtu * max(1UL, ev->ack.acked / cc->algo_state.nr.cwnd);
		path->cwnd = cc->algo_state.nr.cwnd;
		break;
...
{
	chunk_appendf(buf, " state=%s cwnd=%lu ssthresh=%ld recovery_start_time=%lu",
	              quic_cc_state_str(cc->algo_state.nr.state),
	              cc->algo_state.nr.cwnd,
	              (long)cc->algo_state.nr.ssthresh,
	              cc->algo_state.nr.recovery_start_time);
}
