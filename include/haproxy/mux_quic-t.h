...
extern struct pool_head *pool_head_iuqcs;
extern struct pool_head *pool_head_ouqcs;
...
struct iuqcs {
	...
	struct list list; /* To be used when adding in qcc->send_list or qcc->fctl_lsit */
	struct wait_event *subs;  /* recv wait_event the application associated is waiting on (via iuqcs_subscribe) */
};

#define OUQCS_SF_TXBUF_MALLOC 0x00000001
#define OUQCS_SF_TXBUF_FULL   0x00000002
...
struct ouqcs {
	...
	int type;
	uint32_t flags;      /* QC_SF_* */
	struct buffer txbuf; /* Send buffer, always valid (buf_empty or real buffer) */
	struct {
		enum qcs_tx_st st; /* TX state */
		uint64_t max_data; /* maximum number of bytes which may be sent */
		uint64_t offset;   /* the current offset of data to send */
	...
	struct list list; /* To be used when adding in qcc->send_list or qcc->fctl_lsit */
	struct wait_event *subs;  /* recv wait_event the application associated is waiting on (via ouqcs_subscribe) */
};

/* QUIC application layer operations */
struct qcc_app_ops {
	int (*init)(struct qcc *qcc);
	int (*attach_iuqcs)(struct iuqcs *qcs, void *ctx);
